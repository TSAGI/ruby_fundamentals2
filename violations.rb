...
violations = [ {money_owed: 50.0, violation_category: "Garbage and Refuse", date: "2012-11-01 00:00:00", inspection_id: "232528"},
    ...
    violations.each {|hash_element| $violation_categories.push(hash_element[:violation_category])}
    $violation_categories.uniq!
    def num_violations(violations)
        n_of_v = {}
        for category in $violation_categories    
            n_of_v[category] = violations.count {|hash_element| hash_element[:violation_category] == category }
        end
        return n_of_v
    end
    num_violations(violations).each {|k,v| 
        puts "There were #{v} violations in the #{k} category."
    }

    #Problem: Find the total money owed for each violation category
    def money_owed(violations)
        m_o = {}
        for category in $violation_categories
            total_owed = 0
            violations.each {|hash_element| 
                if hash_element[:violation_category] == category
                    total_owed += hash_element[:money_owed]        
                end
            }
            m_o[category] = total_owed
        end
        return m_o
    end
    money_owed(violations).each {|k,v|
        puts "The total money owed in the #{k} category is $%.2f" % v
    }